...
class FTPClient:
    ...
    def read_file(self):
        ...
        command_list = {'command': 'list_files'}
        response_list = self.send_command(command_list)
        
        if response_list['status'] != 'success':
            print(f"ERROR {response_list['message']}")
            return
        
        files = response_list.get('files', [])
        if not files:
    ...
    def download(self):
        ...
        if response['status'] == 'success':
            ...
            with open(local_path, 'w') as f:
                f.write(content)
            print(f"OK File downloaded and saved to {LOCAL_FILES_DIR}/{filename}")
        else:
            print(f"ERROR {response['message']}")
