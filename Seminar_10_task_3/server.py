    ...
    try:
        while True:
            ...
            try:
                ...
                # File operations with history logging
                    ...
                    if os.path.exists(old_path):
                        ...
                        # Log under new filename (old name operation)
                        log_history(new_name, f"renamed from {old_name}")
                        # Also create history for old file? optional, but we keep simple
                    ...
                    if os.path.exists(filepath):
                        with open(filepath, 'r') as f:
                            content = f.read()
                        response = {'status': 'success', 'message': 'File downloaded', 'content': content, 'filename': filename}
                    ...
                    if os.path.exists(history_file):
                        with open(history_file, 'r') as f:
                            history = f.read()
                        response = {'status': 'success', 'message': history}
                    else:
                    ...
                    # Filter out .history files from listing
                    files = [f for f in files if not f.endswith('.history')]
